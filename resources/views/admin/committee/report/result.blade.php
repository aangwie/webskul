        ...
        <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 20px;">
            <div>
                ...
                <div style="display: flex; gap: 30px; flex-wrap: wrap; color: var(--text-light);">
                    ...
                    @if($committeeFee)
                    <span><i class="fas fa-money-bill-wave"></i> Nominal: Rp {{ number_format($committeeFee->amount, 0, ',', '.') }}</span>
                    @endif
                </div>
            </div>
            <div style="display: flex; gap: 10px;">
                <form action="{{ route('admin.committee.report.pdf') }}" method="POST" target="_blank" style="display: inline;">
                    @csrf
...
<div style="display: flex; gap: 20px; margin-bottom: 25px;">
    <div style="flex: 1; background: rgba(40, 167, 69, 0.1); padding: 20px; border-radius: 12px; text-align: center; border: 2px solid var(--success);">
    ...
    <div style="flex: 1; background: rgba(255, 193, 7, 0.1); padding: 20px; border-radius: 12px; text-align: center; border: 2px solid var(--warning);">
...
<div class="card">
    <div class="card-header">
        <h2>
            ...
            @if($reportType == 'class_summary') Recap Per Kelas @elseif($reportType == 'all_summary') Recap Semua Kelas @else Data {{ $reportType == 'detail' ? 'Detail' : 'Rekapitulasi' }} @endif
        </h2>
    </div>
    <div class="card-body">
        <div class="table-responsive">
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td style="text-align: right; color: var(--success); font-weight: 600;">Rp {{ number_format($data['total_paid'], 0, ',', '.') }}</td>
                        <td style="text-align: right; color: var(--danger); font-weight: 600;">Rp {{ number_format($data['remaining'], 0, ',', '.') }}</td>
                    </tr>
                    @endforeach
                </tbody>
                <tfoot style="background: var(--accent); font-weight: 700;">
                    <tr>
                        ...
                        <td style="text-align: right;">Rp {{ number_format($summary['total_tagihan'], 0, ',', '.') }}</td>
                        <td style="text-align: right; color: var(--success);">Rp {{ number_format($summary['total_terbayar'], 0, ',', '.') }}</td>
                        <td style="text-align: right; color: var(--danger);">Rp {{ number_format($summary['total_sisa'], 0, ',', '.') }}</td>
                    </tr>
                </tfoot>
            </table>
            @elseif($reportType == 'all_summary')
            ...
            <div style="padding: 30px; text-align: center; background: var(--accent); border-radius: 15px;">
                <h3 style="margin-bottom: 25px; color: var(--primary);">Ringkasan Dana Komite TA {{ $academicYear->year }}</h3>
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; max-width: 600px; margin: 0 auto; text-align: left;">
                    ...
                    <div style="background: white; padding: 15px; border-radius: 10px;">
                        ...
                        <strong style="font-size: 1.2rem;">Rp {{ number_format($summary['total_tagihan'], 0, ',', '.') }}</strong>
                    </div>
                    <div style="background: white; padding: 15px; border-radius: 10px;">
                        <span style="color: var(--text-light); display: block; font-size: 0.9rem;">Total Terbayar</span>
                        <strong style="font-size: 1.2rem; color: var(--success);">Rp {{ number_format($summary['total_terbayar'], 0, ',', '.') }}</strong>
                    </div>
                    <div style="background: white; padding: 15px; border-radius: 10px;">
                        <span style="color: var(--text-light); display: block; font-size: 0.9rem;">Total Sisa Tagihan</span>
                        <strong style="font-size: 1.2rem; color: var(--danger);">Rp {{ number_format($summary['total_sisa'], 0, ',', '.') }}</strong>
                    </div>
                </div>
                ...
                <div style="margin-top: 30px; max-width: 600px; margin-left: auto; margin-right: auto;">
                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                        ...
                        <span style="color: var(--success); font-weight: 700;">{{ number_format($totalPercent, 1) }}%</span>
                    </div>
                    <div style="width: 100%; background: white; border-radius: 15px; height: 15px; overflow: hidden; border: 1px solid #ddd;">
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td style="text-align: right;">Rp {{ number_format($committeeFee->amount, 0, ',', '.') }}</td>
                        <td style="text-align: right; color: var(--success); font-weight: 600;">Rp {{ number_format($data['total_paid'], 0, ',', '.') }}</td>
                        <td style="text-align: right; color: var(--danger); font-weight: 600;">Rp {{ number_format($data['remaining'], 0, ',', '.') }}</td>
            ...
            <div style="margin-bottom: 30px; padding: 20px; background: var(--accent); border-radius: 12px;">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
                    <div>
                        ...
                        <span style="color: var(--text-light); margin-left: 10px;">NIS: {{ $data['student']->nis ?? '-' }}</span>
                    </div>
                    @if($data['is_paid_full'])
                    <span class="badge badge-success">LUNAS</span>
                    @else
                ...
                <table style="margin-bottom: 10px;">
                    ...
                    <tbody>
                        ...
                        <tr style="background: white;">
                            ...
                            <td style="text-align: right; color: var(--success); font-weight: 600;">Rp {{ number_format($payment->amount, 0, ',', '.') }}</td>
                            <td>{{ $payment->notes ?? '-' }}</td>
                        </tr>
                        @endforeach
                    </tbody>
                ...
                <div style="display: flex; gap: 30px; padding-top: 10px; border-top: 1px dashed #ddd; margin-top: 10px;">
                    <span>Tagihan: <strong>Rp {{ number_format($committeeFee->amount, 0, ',', '.') }}</strong></span>
                    <span>Terbayar: <strong style="color: var(--success);">Rp {{ number_format($data['total_paid'], 0, ',', '.') }}</strong></span>
                    <span>Sisa: <strong style="color: var(--danger);">Rp {{ number_format($data['remaining'], 0, ',', '.') }}</strong></span>
                </div>
            </div>
            @endforeach
            @endif
