...
@section('content')
    <div class="card" style="max-width: 600px;">
        <div class="card-header">
            <h2><i class="fas fa-edit"></i> Edit Data Sumbangan</h2>
        </div>
        <div class="card-body">
            <div style="background: var(--accent); padding: 15px; border-radius: 8px; margin-bottom: 25px;">
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                    ...
                    <div>
                        ...
                        <div style="font-weight: 600; color: var(--success);">Rp
                            {{ number_format($maxAmount, 0, ',', '.') }}</div>
                    </div>
                </div>
            </div>
            ...
            <form action="{{ route('admin.committee.payments.update', $committeePayment->id) }}" method="POST">
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="amount" class="form-input @error('amount') is-invalid @enderror"
                        value="{{ old('amount', $committeePayment->amount) }}" max="{{ $maxAmount }}" min="1" required>
                    @error('amount')
