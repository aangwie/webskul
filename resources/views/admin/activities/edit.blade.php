        ...
        <form action="{{ route('admin.activities.update', $activity) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select name="category" class="form-select" required>
                    <option value="news" {{ old('category', $activity->category) == 'news' ? 'selected' : '' }}>Berita</option>
                    <option value="event" {{ old('category', $activity->category) == 'event' ? 'selected' : '' }}>Acara</option>
                </select>
            </div>

            <div class="form-group">
                <label class="form-label">Konten *</label>
                <textarea name="content" class="form-textarea" rows="8" required>{{ old('content', $activity->content) }}</textarea>
            ...
            <div class="form-group">
                ...
                @if($activity->image)
                    <img src="{{ asset('storage/' . $activity->image) }}" alt="Current Image" class="preview-image" style="max-width: 200px;">
                @endif
            </div>
