...
@section('content')
                        ...
                        @foreach($archives as $archive)
                            <tr>
                                ...
                                <td><span class="badge badge-info"
                                        style="background: rgba(23, 162, 184, 0.1); color: #17a2b8; border: 1px solid #17a2b8;">{{ $archive->archiveType->name }}</span>
                                </td>
                                ...
                                <td>
                                    <div style="display: flex; gap: 5px;">
                                        @php
                                            ...
                                            // Use Storage Proxy Route instead of direct asset() to bypass symlink issues
                                            $fileUrl = $isBase64 ? $archive->file_path : route('admin.storage.view', ['path' => $archive->file_path]);
                                        @endphp
                                        <button type="button" class="btn btn-success btn-sm" title="Lihat"
                                            onclick="previewFile('{{ $fileUrl }}', '{{ addslashes($archive->title) }}')">
                                            <i class="fas fa-eye"></i>
                                        </button>
                                        <a href="{{ $fileUrl }}" download="{{ $archive->title }}" class="btn btn-info btn-sm"
                                            title="Download">
                                            <i class="fas fa-download"></i>
                                        </a>
                                        <button class="btn btn-warning btn-sm" onclick='showEditModal(@json($archive))'
                                            title="Edit">
    ...
    <!-- Modal Tambah/Edit -->
    <div id="archiveModal" class="sidebar-overlay"
            ...
            <form id="archiveForm" method="POST" enctype="multipart/form-data">
                ...
                <div class="card-body">
                    <div class="form-group">
                        ...
                        <input type="text" name="title" id="archiveTitle" class="form-input" required
                            placeholder="Contoh: Sertifikat Pelatihan IT 2024" value="{{ old('title') }}">
                    </div>
                    <div class="form-group">
                        ...
                        <select name="archive_type_id" id="archiveTypeId" class="form-select" required>
                            ...
                            @foreach($types as $type)
                                <option value="{{ $type->id }}" {{ old('archive_type_id') == $type->id ? 'selected' : '' }}>
                                    {{ $type->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group">
                        <label class="form-label">File Arsip <span id="fileRequiredStar"
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="description" id="archiveDescription" class="form-textarea"
                            placeholder="Tambahkan catatan singkat jika perlu...">{{ old('description') }}</textarea>
                    </div>
                </div>
