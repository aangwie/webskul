...
@section('content')
    <div class="card">
        <div class="card-header">
            <h2><i class="fas fa-tags"></i> Manajemen Jenis Buku</h2>
            <button class="btn btn-primary" onclick="showAddModal()">
                <i class="fas fa-plus"></i> Tambah Jenis Buku
            </button>
        </div>
        <div class="card-body">
                    ...
                    <tbody>
                        @forelse($types as $type)
                            <tr>
                                ...
                                <td>{{ $type->name }}</td>
                                <td>{{ $type->books->count() }} buku</td>
                                <td>
                                    <div style="display: flex; gap: 5px;">
                                        <button class="btn btn-warning btn-sm" onclick='showEditModal(@json($type))'>
                                            <i class="fas fa-edit"></i>
                                        </button>
                                        <form action="{{ route('admin.library.book-types.destroy', $type) }}" method="POST"
                                            onsubmit="return confirm('Yakin ingin menghapus jenis buku ini?')">
                                            @csrf
                                            @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="text-center">Belum ada jenis buku</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
    ...
    <!-- Modal Tambah/Edit -->
    <div id="typeModal" class="sidebar-overlay"
        ...
        <div class="card" style="width: 100%; max-width: 500px; margin: 20px;">
            <div class="card-header">
                <h2 id="modalTitle">Tambah Jenis Buku</h2>
...
@section('scripts')
    <script>
        function showAddModal() {
            document.getElementById('modalTitle').innerText = 'Tambah Jenis Buku';
            document.getElementById('typeForm').action = "{{ route('admin.library.book-types.store') }}";
        ...
        function showEditModal(type) {
            document.getElementById('modalTitle').innerText = 'Edit Jenis Buku';
            document.getElementById('typeForm').action = `/admin/library/book-types/${type.id}`;
